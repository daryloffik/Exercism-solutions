...
export function correctListOfWagons(ids, missingWagons) {
  const [id, ...remainingIds] = ids
  const newIds = [id, ...missingWagons, ...remainingIds]
  return newIds
}
...
export function separateTimeOfArrival(information) {
  const { timeOfArrival, ...from} = information
  return [timeOfArrival, {...from}]
}
